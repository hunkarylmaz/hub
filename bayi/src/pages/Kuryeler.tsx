...
function KuryeModal({ kurye, onClose, onSave }: { kurye?: Kurye; onClose: () => void; onSave: () => void }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      else await api.kuryeler.create({ ad: form.ad, telefon: form.telefon || undefined })
      onSave(); onClose()
...
export default function Kuryeler() {
  ...
  return (
    <div>
      {/* Modals */}
      {modal?.type === 'yeni' && <KuryeModal onClose={() => setModal(null)} onSave={fetchData} />}
                  ...
                  <tr key={k.id} className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-sm font-medium text-gray-800 whitespace-nowrap">{k.ad}</span>
                      </div>
                    </td>
                    {/* Telefon */}
